function JSButton(x, y, w, h, label, name, tooltip_text, fonDown, fonUp, fonDbleClick, N_img, H_img, btn_index, state, hover_color, hover_bar_bottom) {
	...
	this.draw = function (gr, x, y) {
		...
		if (this.name == "Foobar") {
			if (layout_state.isEqual(0)) gr.FillSolidRect(this.x + this.w - 1, 0, 1, this.h, colors.settings_btn_line);
			else {
		...
		if (this.label == "Foobar" || (this.label == "" && (this.state == ButtonStates.hover || this.state == ButtonStates.down) || main_panel_state.isEqual(this.btn_index)) && !this.hover_color && !((this.name == "search" || this.name == "fullscreen" || this.name == "lightswitch" || this.name == "nowplaying" || this.name == "rightsidebar") && layout_state.isEqual(0))) {
			btn_opacity = 255;
			text_color = colors.normal_txt;
		}

		else if (this.label == "") btn_opacity = 255;

		else {
			btn_opacity = colors.btn_inactive_opacity;
			text_color = colors.inactive_txt;
		}
