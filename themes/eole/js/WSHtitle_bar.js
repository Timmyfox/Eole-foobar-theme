...
var properties = {
	...
	showMiniBtn: window.GetProperty("_DISPLAY: show mini btn", true),
...
function build_buttons() {
	if (btn_initialized) {
		...
		buttons.Max.H_img = images.minimode_off_img;
		buttons.Max.N_img = images.minimode_off_img;
		...
		buttons = {
			...
			Mini: new JSButton(-112, btn.top_m, btn.width_small_btns, btn.height, "", "mini", "Mini player", function () {
				toggleLayoutMode(); get_colors(); g_searchbox.adapt_look_to_layout();
			}, false, false, images.mini_icon, images.mini_icon, -1, false, false, true),
			...
			}, false, images.minimode_off_img, images.minimode_off_img, -1, false, colors.titlebar_btn_hover_bg, true),
		}
		...
		window_btns.addButtons([buttons.Max], [0, 0, 0, 0]);

		additional_btns = new JSButtonGroup("top-right", 11, btn.top_m, 'additional_btns', true);
		additional_btns.addButtons([buttons.NowPlaying, buttons.ConfigLayout, buttons.RightSidebar, buttons.Fullscreen, buttons.Mini, buttons.Lightswitch, buttons.ShowSearch], [0, 9, 0, 0]);
	}
}
...
function toggleLayoutMode(new_layout_state, main_window_state) {
	...
	if (new_layout_state == 1) {
		...
		try {
			g_uihacks.setMinHeight(Math.max(properties.fullMode_pmanagerheight + (mini_controlbar.isActive() ? (showtrackinfo_big.isActive() ? properties.compactcontrolsHeight : properties.compactNoTrackcontrolsHeight) : properties.fullcontrolsHeight) + (properties.full_titlebar_height), globalProperties.fullMode_minheight) + 1);
		} catch (e) {
...
function adapt_buttons_to_layout() {
	if (layout_state.isEqual(1)) {
		...
		additional_btns.x = 5;
		additional_btns.y = btn.top_m_mini;
		...
		if (!properties.alwaysShowSearch && g_searchbox.inputbox.text.length == 0) {
			g_searchbox.toggleVisibility(false);
		} else g_searchbox.toggleVisibility(true);

		if (!properties.showMiniBtn)
			buttons.Mini.setVisibility(false);
		else buttons.Mini.setVisibility(true);
		...
		additional_btns.y = btn.top_m;
		additional_btns.setBtnsHeight(btn.height);
		additional_btns.setBtnsWidth(btn.width_small_btns);

		window_btns.setBtnsHeight(29);

		cSearchBoxMain.marginRight = cSearchBoxMain.marginRight = 18 + additional_btns.getWidth(true) - (!properties.alwaysShowSearch ? buttons.ShowSearch.w + 5 : 0);

		if (!properties.showRightSidebarBtn && !properties.showLightswitchBtn && !properties.showFullscreenBtn && !properties.showNowPlayingBtn && !properties.showConfigLayoutBtn && !properties.showMiniBtn) {
			cSearchBoxMain.marginRight -= 10;
			cSearchBoxMain.marginRight -= 10;
		}
...
function draw_settings_menu(x, y) {
	...
	// _menu.AppendMenuItem(MF_STRING, 1800, "Display played track title on top");
	// _menu.CheckMenuItem(1800, properties.tracktitle_ontop)
	// _menu.AppendMenuItem(MF_STRING, 1809, "Played track title format...");
	...
	_menu.AppendMenuItem(MF_STRING, 1801, "Always show the search box");
	...
	_menu_button.AppendMenuItem(MF_STRING, 1802, "Mini player");
	_menu_button.CheckMenuItem(1802, properties.showMiniBtn);
	...
	switch (true) {
		...
		case (idx == 1810):
			...
			break;
		case (idx == 1802):
			properties.showMiniBtn = !properties.showMiniBtn;
			window.SetProperty("_DISPLAY: show mini btn", properties.showMiniBtn);
			adapt_buttons_to_layout();
			window.Repaint();
			break;
		case (idx == 1809):
...
oSearch = function () {
	...
	this.adapt_look_to_layout = function () {
		...
		if (layout_state.isEqual(1)) {
			...
			switch (true) {
				...
				default:
					for (var c in cSearchBoxMain)
						if (cSearchBoxMain.hasOwnProperty(c))
							cSearchBox[c] = cSearchBoxMain[c]
					break;
			}
		}
		this.on_size();
	...
	this.draw = function (gr, x, y) {
		...
		if (this.inputbox.text.length > 0 || (!properties.alwaysShowSearch && layout_state.isEqual(0))) {
			this.reset_bt.draw(gr, this.x + cSearchBox.paddingLeft - this.images.search_icon.Width - 1, this.y + Math.floor((this.h - cSearchBox.paddingBottom) / 2 - this.reset_bt.img[0].Height / 2) + 2, 255);
		} else {
	...
	this.resetSearch = function () {
		...
		this.clearInputbox(true);
		if (!properties.alwaysShowSearch && layout_state.isEqual(0)) g_searchbox.toggleVisibility(false);
		g_searchbox.repaint();
	}
	this.on_mouse = function (event, x, y, delta) {
		switch (event) {
			case "lbtn_down":
				...
				else if (!properties.alwaysShowSearch && this.inputbox.text.length == 0 && layout_state.isEqual(0) && !this.hide) {
				...
				break;
			case "lbtn_up":
				if ((this.inputbox.text.length > 0 || (!properties.alwaysShowSearch && layout_state.isEqual(0))) && !this.hide) {
					if (this.reset_bt.checkstate("up", x, y) == ButtonStates.hover && !this.inputbox.drag) {
						this.resetSearch();
					};
				} else if (properties.alwaysShowSearch) {
			...
			case "move":
				...
				if ((this.inputbox.text.length > 0 || (!properties.alwaysShowSearch && layout_state.isEqual(0))) && !this.hide) this.reset_bt.checkstate("move", x, y);
	...
	this.on_focus = function (is_focused) {
		...
		if (!is_focused && !properties.alwaysShowSearch && this.inputbox.text.length == 0 && layout_state.isEqual(0)) {
			this.toggleVisibility(false);
		}
